...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // --- MULAI SINTAKS PEMBARUAN LOGIKA DATA VARIASI ---
    ...
    for ($i = 0; $i < count($list_harga); $i++) {
        ...
        if (trim($list_berat[$i]) === "" || trim($list_harga[$i]) === "") {
            continue;
        }
    ...
    // --- SELESAI SINTAKS PEMBARUAN LOGIKA DATA VARIASI ---

    // --- LOGIKA UPLOAD FOTO PRODUK ---
    $target_dir = "../pictures/"; // Keluar folder aksesAdmin untuk taruh file fisik ke folder pictures
    ...
    if (!in_array($ekstensi, $ekstensi_diperbolehkan)) {
        ...
        // Membuat nama file acak yang unik agar tidak saling menimpa jika nama fotonya sama
        $nama_file_baru = time() . "_" . uniqid() . "." . $ekstensi;
        $target_file = $target_dir . $nama_file_baru; // Path fisik untuk memindahkan file

        // PATH INI YANG DISIMPAN KE DATABASE (Bersih tanpa tanda "../" agar bisa dibaca dari luar)
        $path_database = "pictures/" . $nama_file_baru;

        if (
            move_uploaded_file($_FILES["foto_produk"]["tmp_name"], $target_file)
        ) {
            // Gunakan $path_database di dalam Query INSERT
            $query = "INSERT INTO produk (nama_produk, harga_perProduk, berat_produk, stok_tersedia, deskripsi, foto_produk) 
                      VALUES ('$nama', 'Multi Harga', '$variasi_final', '$stok', '$deskripsi', '$path_database')";
            ...
            if (mysqli_query($conn, $query)) {
                ...
                // addslashes() mengamankan tanda kutip pesan error SQL agar tidak merusak JavaScript SweetAlert
                $alert_message =
            ...
            $alert_status = "gagal_upload";
            $alert_message =
                "Terjadi kendala teknis saat mengunggah berkas foto produk.";
        }
    }
}
?>
...
<script>
    ...
    // --- INTEGRASI POPUP MODERN DARI PHP STATUS ---
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if (status === "sukses") {
            Swal.fire({
                ...
                text: msg,
                icon: 'success',
                confirmButtonText: '<i class="fa-solid fa-check me-2" style="color: white;"></i>&nbsp;&nbsp;<span style="color: white;">Selesai</span>',
                customClass: {
                    popup: 'swal2-popup',
                    title: 'swal2-title',
