...
// Proteksi Halaman Admin (Aktifkan jika sistem login sudah siap)
...
if (isset($_POST["update"])) {
    ...
    for ($i = 0; $i < count($list_berat); $i++) {
        if (!empty($list_berat[$i]) && !empty($list_harga[$i])) {
            $variasi_final[] = $list_berat[$i] . "(" . $list_harga[$i] . ")";
        }
    }
    ...
    if ($_FILES["foto_produk"]["name"] != "") {
        $nama_file = time() . "_" . $_FILES["foto_produk"]["name"];
        $tmp_name = $_FILES["foto_produk"]["tmp_name"];
        $folder = "pictures/" . $nama_file;
        move_uploaded_file($tmp_name, $folder);
        $foto_final = $folder;

        // Hapus foto lama jika ada dan bukan gambar default
        if (file_exists($foto_lama) && $foto_lama != "" && $foto_lama != "pictures/default.png") {
            unlink($foto_lama);
        }
    } else {
...
<body>
                    ...
                    <form action="" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <div id="container-variasi">
                                <?php 
                                // Memecah string data variasi "1kg(80000), 0.5kg(40000)" menjadi pecahan input form
                                ...
                                <div class="row g-2 mb-2 variasi-item align-items-center">
                                    <div class="col-5">
                                        <input type="text" name="berat_input[]" class="form-control form-control-sm" placeholder="Berat (ex: 1kg atau 1 Ekor)" value="<?= $berat_val ?>" required>
                                    </div>
                                    <div class="col-5">
                                        <div class="input-group input-group-sm">
                                            <span class="input-group-text">Rp</span>
                                            <input type="number" name="harga_input[]" class="form-control" placeholder="Harga tanpa titik (ex: 85000)" value="<?= $harga_val ?>" required>
                                        </div>
                                    </div>
                        ...
                        <div class="row g-4 mb-5">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="d-flex align-items-center gap-3 p-3 border rounded-4 bg-light">
                                    <img src="<?= $data["foto_produk"] ?>" class="img-preview shadow-sm" alt="Foto Aktif">
                        ...
                        <div class="d-flex justify-content-end gap-3 pt-3 border-top">
                            <a href="product.php" class="btn btn-custom btn-batal text-decoration-none d-flex align-items-center">Batalkan</a>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // Event Tambah Baris Variasi Baru
            ...
            // Event Hapus Baris Variasi (Delegasi Event)
            container.addEventListener("click", function(e) {
                if (e.target.closest(".btn-hapus-variasi")) {
                    const baris = e.target.closest(".variasi-item");
                    // Pastikan tidak menghapus jika hanya tersisa 1 baris agar form tidak kosong melompong
