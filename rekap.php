...
<body>
    ...
    <div class="container-fluid p-4 p-md-5">
        ...
        <div class="main-header p-4 mb-4 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
            ...
            <div class="d-flex align-items-center gap-3">
                ...
                <a href="index.php" class="btn btn-light fw-bold text-dark btn-custom-action px-3 py-2 shadow-sm">
                ...
                <table class="table table-bordered align-middle">
                    <thead class="text-center">
                        <tr>
                            ...
                            <th style="width: 220px;">Aksi Berkas</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        ...
                        if (mysqli_num_rows($query) > 0):
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="fw-bold text-dark fs-6 mb-1"><?= $data["nama_produk"] ?></div>
                                    <div class="d-flex gap-1 flex-wrap">
                                        <span class="badge badge-info-custom">Ukuran: <?= $data["ukuran"] ?></span>
                                        <span class="badge bg-secondary text-white small">ID: #<?= $data["id_pesanan"] ?></span>
                                    </div>
                                </td>
                                <td class="text-center">
                                    <?php if ($data["no_hp"]): ?>
                                        <a href="https://wa.me/<?= $data["no_hp"] ?>" target="_blank" class="btn btn-sm btn-outline-success fw-bold px-3 py-1">
                                            <i class="fa-brands fa-whatsapp me-1"></i> <?= $data["no_hp"] ?>
                                        </a>
                                    <?php else: ?>
                                ...
                                <td class="fw-bold text-end text-success fs-6">
                                    Rp <?= number_format($total_bayaran, 0, ",", ".") ?>
                                </td>
                                <td class="text-center">
                                    <?php if ($data["status"] == "Pending"): ?>
                                        <span class="status-badge-pending small">Pending</span>
                                    <?php else: ?>
                                        <span class="status-badge-selesai small">Selesai</span>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <div class="d-flex gap-2 justify-content-center">
                                        ...
                                        <?php if ($data["status"] == "Pending"): ?>
                                            <a href="konfirmasi_proses.php?id=<?= $data["id_pesanan"] ?>&status=Selesai" 
                                               class="btn btn-custom-action btn-success shadow-sm btn-action-selesai">
                                        ...
                                        <div class="modal-dialog modal-lg modal-dialog-centered">
                                            <div class="modal-content" style="border-radius: 12px; overflow: hidden;">
                                                <div class="modal-header modal-header-custom">
                                                    <h5 class="modal-title fw-bold"><i class="fa-solid fa-receipt me-2"></i>Rincian Faktur #<?= $data['id_pesanan'] ?></h5>
                                                    ...
                                                    <div class="row g-4">
                                                        ...
                                                        <div class="col-md-7">
                                                            ...
                                                            <table class="table table-sm table-striped table-bordered mb-4">
                                                                <tr><td class="bg-light fw-bold" style="width: 35%;">Nama Produk</td><td><?= $data['nama_produk'] ?> (<?= $data['ukuran'] ?>)</td></tr>
                                                                <tr><td class="bg-light fw-bold">Harga Satuan</td><td>Rp <?= number_format($data['harga_satuan'], 0, ',', '.') ?></td></tr>
                                                                <tr><td class="bg-light fw-bold">Jumlah Order</td><td><?= $data['jumlah'] ?> Pcs</td></tr>
                                                                <tr><td class="bg-light fw-bold">No. Kontak</td><td><?= $data['no_hp'] ?: '-' ?></td></tr>
                                                                <tr><td class="bg-light fw-bold">Sistem Bayar</td><td><span class="badge bg-dark"><?= $data['metode_pembayaran'] ?></span></td></tr>
                                                            </table>

                                                            <div class="section-divider"><i class="fa-solid fa-map-location-dot me-2"></i>Informasi Logistik</div>
                                                            <table class="table table-sm table-striped table-bordered">
                                                                <tr><td class="bg-light fw-bold" style="width: 35%;">Kurir/Kirim</td><td><?= $data['metode_pengiriman'] ?></td></tr>
                                                                <tr><td class="bg-light fw-bold">Provinsi</td><td><?= $data['provinsi'] ?: '-' ?></td></tr>
                                                                <tr><td class="bg-light fw-bold">Kabupaten</td><td><?= $data['kabupaten'] ?: '-' ?></td></tr>
                                                                <tr><td class="bg-light fw-bold">Alamat Tujuan</td><td><?= $data['alamat_lengkap'] ?: 'Ambil Sendiri di Toko' ?></td></tr>
                                                                <tr><td class="bg-light fw-bold">Nilai Ongkir</td><td class="fw-bold text-danger">Rp <?= number_format($data['biaya_ongkir'], 0, ',', '.') ?></td></tr>
                                                            </table>
                                                        </div>
                                                        ...
                                                        <div class="col-md-5 text-center border-start">
                                                            ...
                                                            <?php if ($data["bukti_pembayaran"]): ?>
                                                                <div class="mb-3">
                                                                    <img src="uploads/<?= $data["bukti_pembayaran"] ?>" class="img-fluid img-bukti shadow-sm" alt="Bukti Transfer">
                                                                </div>
                                                                <a href="uploads/<?= $data["bukti_pembayaran"] ?>" target="_blank" class="btn btn-sm btn-dark w-100 py-2">
                                                            ...
                                                            <div class="mt-4 p-3 bg-dark text-white rounded text-start shadow-inner">
                                                                ...
                                                                <span class="fw-bold"><i class="fa-regular fa-calendar-check me-2 text-warning"></i><?= $data['waktu_transaksi'] ?></span>
                                                            </div>
                                                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // 1. POP UP KONFIRMASI SELESAI
            const selesaiButtons = document.querySelectorAll('.btn-action-selesai');
            selasaiButtons.forEach(button => {
                button.addEventListener('click', function(e) {
                    e.preventDefault(); // Menahan link agar tidak langsung berpindah halaman
                    const targetUrl = this.getAttribute('href');

                    Swal.fire({
                        title: 'Selesaikan Pesanan? 🦆',
                        text: "Pastikan dana pembayaran sudah masuk dan berkas valid.",
                        icon: 'question',
                        iconColor: '#ff9f43',
                        showCancelButton: true,
                        confirmButtonColor: '#155724', // Warna hijau sukses
                        ...
                        buttonsStyling: false // Mematikan style default agar menggunakan class Bootstrap
                    }).then((result) => {
                        if (result.isConfirmed) {
                            window.location.href = targetUrl; // Eksekusi pindah halaman jika klik "Ya"
                        }
                    });
                });
            });

            // 2. POP UP KONFIRMASI HAPUS PESANAN
            ...
            hapusButtons.forEach(button => {
                button.addEventListener('click', function(e) {
                    ...
                    Swal.fire({
                        ...
                        icon: 'warning',
                        iconColor: '#dc3545', // Warna merah danger
                        showCancelButton: true,
